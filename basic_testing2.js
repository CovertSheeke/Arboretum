...
class Deck extends PileOfCards {
  ...
  generateDeck() {
    ...
    if (this.numPlayers === 2) {
      selectedSuits = this.shuffle(this.suitsInfo).slice(0, 6); // 6 suits for 2 players
    } else if (this.numPlayers === 3) {
      selectedSuits = this.shuffle(this.suitsInfo).slice(0, 8); // 8 suits for 3 players
    } else if (this.numPlayers === 4) {
      selectedSuits = this.suitsInfo; // All 10 suits for 4 players
    } else {
...
// Create a deck with 5 cards (shuffled)
const deck = new Deck(2);

console.log("Drawing from the deck:", deck.drawCard()); // Draw a card from the deck

// Create a discard pile and add a card to it
const discardPile = new DiscardPile();
discardPile.addCard({ rank: 1, suit: "Maple", color: "Orange" }); // Add a card to discard pile

console.log("Top card in discard pile:", discardPile.viewTopCard()); // View top card in discard pile
console.log("Drawing from discard pile:", discardPile.drawCard()); // Draw a card from discard pile
console.log("Top card in discard pile:", discardPile.viewTopCard()); // View top card in discard pile
